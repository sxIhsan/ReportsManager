...
FlexmonsterToolbar.prototype.showFormatCellsDialog = function (measureName) {
    ...
    var switchToMeasure = function () {
        updateDropdowns();
        var formatVO = self.pivot.getFormat(valuesDropDown.getValue());
        currentMeasureNames = valuesDropDown.getAllValues();
    ...
    var applyHandler = function () {
        var currentReport = (currentFormatVO.name != "" || valuesDropDown.getValue() == "") ? self.pivot.getReport() : {};
        ...
        var measures = valuesDropDown.getAllValues();
        for (var i = 0; i < measures.length; i++) {
    ...
    for (var i = 0; i < _measures.length; i++) {
        if (_uniqueNames.indexOf(_measures[i].uniqueName) == -1) {
            ...
            measures.push({
                name: _measures[i].name,
                label: _measures[i].uniqueName
            });
        }
    }

    var valuesDropDown = self.createMultiSelect(Labels.choose_value, measures);
...
FlexmonsterToolbar.prototype.createMultiSelect = function (promptLabel, items) {
    var optionsHTML = "";
    var boldStyle = ' style="font-weight: bold;"';
    for (var i = 0; i < items.length; i++) {
        var item = items[i];
        optionsHTML += '<li class="fm-ui-element fm-ui fm-not-selected" tabindex="0" data-index="' + i + '"><span ' +
            'class="fm-ui-element fm-ui fm-ui-label fm-text-display"' + (i == 0 ? boldStyle : "") + '>' + item.label + '</span></li>'
    }
    var wrapper = document.createElement("template");
    wrapper.innerHTML = '<div class="fm-ui-element fm-ui fm-ui-dropdown fm-ui-prompt" ' +
        'style="display: inline-block; width: calc(100% - 160px); vertical-align: middle;"><a ' +
        'class="fm-ui-element fm-ui fm-ui-toggle-btn fm-ui-dropdown-btn" role="button" tabindex="0" ' +
        'title="' + promptLabel + '"><span class="fm-ui-element fm-ui fm-ui-label">' + promptLabel + '</span></a>' +
        '<div class="fm-ui-element fm-ui fm-ui-dropdown-list">' +
        '<ul class="fm-ui-element fm-ui fm-ui-list fm-ui-dropdown-multiple-selection-list" tabindex="-1">' + optionsHTML + '</ul>' +
        '</div>' +
        '</div>'
    var select = wrapper.content !== undefined ? wrapper.content.firstChild : wrapper.firstChild;
    var prompt = select.firstChild;
    var dropdown = select.children[1];
    ...
    dropdown.onclick = function (event) {
        ...
        var selected = item.classList.contains("fm-selected");
        if (index === 0) {
            selectAll(!selected);
        } else {
            setSelected(item, !selected);
            // var all = dropdown.querySelector('li[data-index="0"]');
            // if (selected) {
            //     setSelected(all, false);
            // } else {
            // var allSelected = checkIfAllSelected();
            // if (allSelected) setSelected(all, true);
    ...
    function hide() {
        ...
        prompt.classList.remove('fm-selected');
    }

    function show() {
        dropdown.style.display = 'block';
        prompt.classList.add('fm-selected');
    }

    function setSelected(item, selected) {
        if (selected) {
            item.classList.add("fm-selected");
            item.classList.remove("fm-not-selected");
        } else {
            item.classList.remove("fm-selected");
            item.classList.add("fm-not-selected");
        }
    }

    function selectAll(selected) {
        for (var i = 0; i < items.length; i++) {
            var item = dropdown.querySelector('li[data-index="' + i + '"]');
    ...
    function setPromptValue() {
        ...
        var label = labels.length === 0 ? promptLabel : labels.join(", ");
        prompt.setAttribute("title", label);
        prompt.firstChild.textContent = label;
    }

    select.setValue = function (measures) {
        ...
        if (measures.length === 1 && measures[0] === "") {
            ...
            for (var i = 0; i < measures.length; i++) {
                var idx = items.findIndex(function (x) {
                    return x.name === measures[i]
                });
                if (idx !== -1) {
                    var item = dropdown.querySelector('li[data-index="' + idx + '"]');
    ...
    select.getValue = function () {
        for (var i = 0; i < items.length; i++) {
            var item = dropdown.querySelector('li[data-index="' + i + '"]');
            var selected = item.classList.contains("fm-selected");
            if (selected) return items[i].name;
        }
        return undefined;
    }
    select.getAllValues = function () {
        var values = [];
        for (var i = 0; i < items.length; i++) {
            var item = dropdown.querySelector('li[data-index="' + i + '"]');
            var selected = item.classList.contains("fm-selected");
